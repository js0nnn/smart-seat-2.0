...
static __attribute__((inline)) void I2C_WaitIdle(void){
...
static __attribute__((inline)) void I2C_Start(void){ I2C_WaitIdle(); SEN = 1; while(SEN); }
static __attribute__((inline)) void I2C_Restart(void){ I2C_WaitIdle(); RSEN = 1; while(RSEN); }
static __attribute__((inline)) void I2C_Stop(void){ I2C_WaitIdle(); PEN = 1; while(PEN); }
static __attribute__((inline)) void I2C_Write(uint8_t d){ I2C_WaitIdle(); SSPBUF = d; while(!SSPIF); SSPIF=0; }
static __attribute__((inline)) uint8_t I2C_Read(uint8_t more){
    uint8_t d; I2C_WaitIdle(); RCEN=1; while(!BF); d=SSPBUF; I2C_WaitIdle();
    ACKDT = more ? 0 : 1; ACKEN=1; while(ACKEN); return d;
}
# 90 "main.c"
static unsigned char RS, i2c_add, BackLight_State = 0x08;
...
static void LCD_Init(unsigned char I2C_Add) {
    ...
    LCD_CMD(0x03); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x03); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x03); _delay((unsigned long)((5)*(8000000UL/4000.0)));

    LCD_CMD(0x02); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x20 | (2 << 2)); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x0C); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x01); _delay((unsigned long)((5)*(8000000UL/4000.0)));
    LCD_CMD(0x04 | 0x02); _delay((unsigned long)((5)*(8000000UL/4000.0)));
}
...
static __attribute__((inline)) void MAX_Write(uint8_t reg, uint8_t val){
    I2C_Start();
    I2C_Write((0x57<<1)|0);
    I2C_Write(reg);
    I2C_Write(val);
...
static __attribute__((inline)) uint8_t MAX_Read(uint8_t reg){
    uint8_t v;
    I2C_Start();
    I2C_Write((0x57<<1)|0);
    I2C_Write(reg);
...
static void MAX30102_Init(void){
    MAX_Write(0x09, 0x40); _delay((unsigned long)((100)*(8000000UL/4000.0)));
    MAX_Write(0x08, 0x00);
...
static void process_heart_rate(uint32_t ir){
    ...
    ir_buf[buf_index] = ir;
    buf_index = (uint8_t)((buf_index + 1U) % 8);
    uint32_t sum = 0; for(uint8_t i=0;i<8;i++) sum += ir_buf[i];
    uint32_t avg = sum / 8;
...
if (SystemStatus == 1) {
    ...
    SystemStatus = 0;
        ...
        if (ADCcode == 1){
            ...
            adc5 = ADC_Read(1,1,0,0);
            _delay((unsigned long)((20)*(8000000UL/4000.0)));


            adc6 = ADC_Read(1,0,1,0);
            _delay((unsigned long)((20)*(8000000UL/4000.0)));


            adc8 = ADC_Read(1,0,0,1);
            _delay((unsigned long)((20)*(8000000UL/4000.0)));


            adc9 = ADC_Read(1,0,1,1);
            _delay((unsigned long)((20)*(8000000UL/4000.0)));
            _delay((unsigned long)((200)*(8000000UL/4000.0)));
             if (adc2 >900 && adc3 > 500)
            {
                 RC2=1;
               DHB=0;
                SP=1;
